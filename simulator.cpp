...
void Simulator::fetch() {
  ...
  if (icode == IRRMOVQ || icode == IOPQ || icode == IPUSHQ || icode == IPOPQ ||
  ...
  word valC = 0;
  if (needRegis)
    imem_error = !memory.getWord(selectedPC + 2, &valC);
  else
    imem_error = !memory.getWord(selectedPC + 1, &valC);
  ...
  if (icode >= 0 && icode <= 0xB) {
    switch (icode) {
      case IOPQ:
        instrValid = (ifun >= 0 && ifun <= 3);
        break;
      case IJXX:
      case IRRMOVQ:
        instrValid = (ifun >= 0 && ifun <= 6);
        break;
      default:
        instrValid = (ifun == 0);
    }
...
void Simulator::decode() {
  ...
  if (D.icode == IRRMOVQ || D.icode == IIRMOVQ || D.icode == IOPQ) {
    dstE = D.rB;
  } else if (D.icode == IPUSHQ || D.icode == IPOPQ || D.icode == ICALL ||
             D.icode == IRET) {
    dstE = RRSP;
  } else {
    dstE = 0xF;
  ...
  byte srcA = 0;
  if (D.icode == IRRMOVQ || D.icode == IRMMOVQ || D.icode == IOPQ ||
      D.icode == IPUSHQ) {
    srcA = D.rA;
...
void Simulator::execute() {
  ...
  word ALU_B = 0;
  if (E.icode == IRMMOVQ || E.icode == IMRMOVQ || E.icode == IOPQ ||
      E.icode == ICALL || E.icode == IPUSHQ || E.icode == IRET ||
      E.icode == IPOPQ)
    ALU_B = E.valB;
  ...
  if (setCC) {
    ...
    if ((ALU_A < 0 == ALU_B < 0) && (valE < 0 != ALU_A < 0))
      E.CC[OF] = 1;
    else
      E.CC[OF] = 0;
  }
...
void Simulator::memo() {
  ...
  bool memWrite = false;
  if (M.icode == IRMMOVQ || M.icode == IPUSHQ || M.icode == ICALL)
    memWrite = true;
...
void Simulator::controlLogicUpdate() {
  ...
  control.setCC = (E.icode == IOPQ) && m.stat == AOK && W.stat == AOK;
...
void Simulator::print() {
  ...
  std::cout << std::hex << (int)D.stat << (int)D.icode << (int)D.ifun
            << (int)D.rA << (int)D.rB << D.valC << D.valP << std::endl;
  std::cout << std::hex << (int)E.stat << (int)E.icode << (int)E.ifun << E.valC
            << E.valA << E.valB << (int)E.dstE << (int)E.dstM << (int)E.srcA
            << (int)E.srcB << std::endl;
  std::cout << std::hex << (int)M.stat << (int)M.icode << M.Cnd << M.valE
            << M.valA << (int)M.dstE << (int)M.dstM << std::endl;
  std::cout << std::hex << (int)W.stat << (int)W.icode << W.valE << W.valM
            << (int)W.dstE << (int)W.dstM << std::endl;
}
